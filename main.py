    ...
    for epoch in range(1,epochs+1):
        ...
        for img, label in get_batches_fn(batch_size):
            feed_dict = {keep_prob: keep_prob_value,
                         ...
                         input_image: img,
                         correct_label: label}
            _, loss_result = sess.run((train_op, loss_op), feed_dict=feed_dict)
            #iou, iou_op = tf.metrics.mean_iou(correct_label, last_layer, 2)
            #_, iou_result = sess.run([iou_op, iou])
            #print("  Mean IoU = {}".format(iou_result))
